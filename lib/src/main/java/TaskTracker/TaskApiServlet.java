...
public class TaskApiServlet extends HttpServlet
{
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException 
    {
        ...
        Map<Integer, Task> mappedTasks = tDao.getAll();
        Collection<Task> tasks = mappedTasks.values();
		...
		if(!mappedTasks.containsKey(modelId)) 
        {
			resp.sendError(HttpServletResponse.SC_NOT_FOUND);
			return;
		}
		
		sendAsJson(resp, mappedTasks.get(modelId));
		return;
    }
}
